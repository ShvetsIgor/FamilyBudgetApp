...
- **2026-05-24** — Mobile UX cleanup: deleted dead BottomNav.tsx, MenuOverlay footer safe-area fix (max 32px/env), AGENTS.md changelog entry.
...
- **2026-05-24** — Post-deploy bug fixes: AuthProvider loads folders into Redux on login; seedDefaultCategories is folder-safe with folderId migration for existing users; FastExpenseEntry split picker flat-category fallback + fmtCount() plurals; CategoriesHub replaces ⚡ Конструктор with + Папка / + Категория; .vercelignore excludes design-archive/ from deploys; 427 tests green.
...
- **2026-05-26** — PinnedToday budget overrun fix: `over = spent - total` shown instead of clamped-zero `left`; folder-first two-stage ClarifyCard for unknown merchants (`isTagLearning` mode + inline "Создать папку" flow).
- **2026-05-26** — 12-bug stabilization: chat auto-select fix (storeId with no history → clarify), savings display (removed "Savings:" prefix, paymentMethod card), income localization (t.cat in IncomeCard), budget overflow shows real delta, recurring Type field removed, recurring past-date expense uses startDate, 🔄 indicator only for recurring tags, supermarket split into food+dining presets, constructor folders collapsed by default, reset clears suggestion memory + leaves expense collection empty, ru.json +Dining Out.
- **2026-05-26** — UX redesign: "папка" → "Раздел" everywhere in UI (FolderEditorSheet, CategoryEditorSheet, CategoriesHub, ru.json); FastExpenseEntry chat flow — replaced category grid with folder chips when initialFolderId set, "Уточнить позицию" opens picker to selected folder; ClarifyCard inline folder creation i18n updated.
- **2026-05-25** — Hardening pass: extracted useSplitEditor hook (SplitRow, applyKey, split state + derived math) from FastExpenseEntry; fixed setEditing/requestAnimationFrame side-effects inside setSplits updater (React 18 strict mode); added splitsOverflow detection + red border + disabled save; fixed expenses/page.tsx undo-delete timer leak on unmount; 483 tests green, build clean.

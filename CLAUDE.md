...
- **2026-07-28** — Payment kind and category merged into one choice. Asking for the kind and then for a category was the same question twice, so the preset library gained a **«Регулярные» folder** holding a category per kind (`rec_subscription`, `rec_rent`, `rec_utility`, `rec_credit`, `rec_mortgage`, `rec_installment`, `rec_other`) — ordinary library entries the user can rename, delete or extend. Tapping a kind chip now selects that category; the old required «CHOOSE SECTION» card became a quiet «Другая категория» link that opens the canonical picker, and it only turns back into a card when a real override is set. `typeCategory.ts` remaps the kinds accordingly and gains `typeForCategoryId`, so editing an existing payment lights up the right chip; `hasOverride` distinguishes «the kind's category» from «something else the user chose», which also keeps the chip state correct in edit mode. The three leaves added for the previous semantic mapping (`sub_generic`, `credit_payment`, `installment_pay`) are removed — they existed only to serve it. This **reverses the 2026-07-27 decision** to route kinds into semantic folders (rent → Home/Rent): that kept analytics tidier but made the form ask twice, and after using it the product call was one choice. No migration: existing test data is being wiped. Verified in-browser: «Квартира ₪500» with kind Rent lands in the RENT category and the folder shows up in Categories, editable. Baseline: lint clean, tsc clean, unit **612**, build clean.
- **2026-07-28** — Currencies stopped being added together. A per-template currency (added for recurring payments) meant one account could hold a $12 charge next to ₪ spend, and every personal total summed `amount` blindly — the $12 became ₪12 in the month header, the budget bar, `monthlyStats` and analytics. **No conversion** (there is no FX source on the free plan): amounts are grouped per currency instead. Write path: `buildStatsDelta` carries the expense currency and `monthlyStats` gains `totalsByCurrency` (nested map, so `merge:true` preserves other currencies); income gains `incomeByCurrency`, and `buildIncomeStatsDeltas` treats a currency change as a move between buckets, like a month change. Expense delta merging is keyed by month **and** currency, since an edit can change the currency. `totalExpenses`/`totalIncome` stay as legacy blind sums for pre-existing docs. Read path: new `shared/utils/currencyTotals.ts` (the family helpers moved here, `familyCurrency` re-exports them) adds `splitOwnCurrency`; `/expenses` shows the account-currency total big with foreign totals beneath and per-day totals per currency; `/home` and `/budget` compute budget maths in the account currency only, with `/budget` listing foreign spend under the bar; `/analytics` and `/statistics` normalize stats through new `toOwnCurrency`, and analytics notes what is excluded via `foreignTotals`. A test caught a real flaw mid-way: `totalsByCurrency?.[currency] ?? totalExpenses` fell back to the blind sum when the breakdown existed but lacked that currency — relabelling someone else's money; the fallback now applies only when the breakdown is absent entirely. Known limit, documented in code: `byCategory` remains a blind sum, so a category holding foreign spend is overstated in the category breakdown while the totals people read are exact. Tests: `utils.currencyTotals` (10), income deltas extended. Baseline: lint clean, tsc clean, unit **610**, build clean.

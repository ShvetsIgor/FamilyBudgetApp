...
export default function AccountPage() {
  ...
  async function handleExport() {
    ...
    try {
      ...
      if (exportType === 'expenses' || exportType === 'both') {
        ...
        if (data.length > 0) {
          downloadCsv(expensesToCsv(data, catNames), `expenses-${exportMonth}.csv`);
        }
      }
      if (exportType === 'income' || exportType === 'both') {
        const data = await fetchMonthIncome(user.id, exportMonth);
        if (data.length > 0) {
          downloadCsv(incomeTocsv(data, catNames), `income-${exportMonth}.csv`);
        }
      }
    } finally {
      setExporting(false);
  ...
  async function handleCreateFamily() {
    ...
    try {
      ...
      setFamilyError('Failed to create family. Try again.');
    } finally {
  ...
  async function handleSendInvite() {
    ...
    try {
      ...
      setFamilyError('Failed to send invite. Try again.');
    } finally {
      setFamilyLoading(false);
    }
  }
  ...
  async function handleAcceptInvite() {
    ...
    try {
      ...
      if (f) {
        ...
        const m = await fetchFamilyMembers(f.memberIds);
        dispatch(setMembers(m));
      }
      dispatch(setUser({ ...user!, familyId: pendingInvite.familyId, accountType: 'family' }));
      dispatch(setPendingInvite(null));
    } catch {
      setFamilyError('Failed to accept invite. Try again.');
    } finally {
      setFamilyLoading(false);
    }
  }
  ...
  async function handleRejectInvite() {
    ...
    setFamilyLoading(true);
    try {
      await rejectInvite(pendingInvite.id);
      dispatch(setPendingInvite(null));
    } finally {
      setFamilyLoading(false);
    }
  ...
  async function handleLeaveFamily() {
    if (!family) return;
    const msg = isOwner
      ? 'You are the owner. Leaving will dissolve the family for all members. Continue?'
      : 'Leave this family?';
    ...
    try {
      ...
      setFamilyError('Failed to leave family. Try again.');
    } finally {
      setFamilyLoading(false);
    }
  }
          ...
          {editingName ? (
            ...
            <div className="flex items-center gap-2">
              ...
              <button
                ...
                className="text-muted-foreground hover:text-primary transition-colors text-xs shrink-0"
              >
                ✎
              </button>
            </div>
      ...
      {family ? (
        <div className="rounded-2xl border border-border bg-card p-4 flex flex-col gap-3">
          <div className="flex items-center justify-between">
            ...
            {isOwner && (
              <span className="text-xs bg-primary/10 text-primary px-2 py-0.5 rounded-full">{t('account.owner')}</span>
            )}
          </div>
            ...
            {members.map((m) => {
              ...
              return (
                <div key={m.id} className="flex items-center gap-3">
                  ...
                  {m.id === family.ownerId && (
                    <span className="ml-auto text-xs text-muted-foreground shrink-0">{t('account.owner')}</span>
                  )}
                </div>
              );
            })}
          ...
          {isOwner && (
            <>
              {inviteSent && (
                <p className="text-xs text-green-600 bg-green-50 dark:bg-green-950/30 rounded-lg px-3 py-2">
                  {t('account.inviteSent')}
                </p>
              )}
              {showInvite ? (
                <div className="flex flex-col gap-2">
                  ...
                  <div className="flex gap-2">
                    <button
                      ...
                      disabled={!inviteEmail.trim() || familyLoading}
          ...
          {familyError && (
            <p className="text-xs text-destructive bg-destructive/10 rounded-lg px-3 py-2">{familyError}</p>
          )}
          ...
          {showCreateFamily ? (
            <div className="flex flex-col gap-2">
              ...
              <div className="flex gap-2">
                ...
                <button
                  onClick={() => { setShowCreateFamily(false); setFamilyName(''); }}
                  className="rounded-xl border border-border px-4 py-2.5 text-sm text-muted-foreground"
                >
                  {t('common.cancel')}
                </button>
          ...
          {(['light', 'dark'] as Theme[]).map((th) => (
            <button
              ...
              className={cn(
                ...
                theme === th ? 'bg-card shadow-sm text-foreground' : 'text-muted-foreground'
              )}
            >
        ...
        <div className="grid grid-cols-2 gap-2">
          {CURRENCIES.map((c) => (
            <button
              ...
              className={cn(
                'rounded-xl py-2.5 text-sm font-medium transition-colors border',
                currency === c.value
        ...
        <div className="flex flex-col gap-2">
          {LANGUAGES.map((l) => (
            <button
              ...
              className={cn(
                'flex items-center justify-between rounded-xl px-4 py-2.5 text-sm font-medium transition-colors border',
                language === l.value
                  ? 'border-primary bg-primary/10 text-primary'
                  : 'border-border text-muted-foreground'
              )}
            ...
            {(['expenses', 'income', 'both'] as const).map((type) => (
              <button
                ...
                className={cn(
                  ...
                  exportType === type ? 'bg-card shadow-sm text-foreground' : 'text-muted-foreground'
                )}
              >
                {t(`export.${type}`)}
              </button>

    ...
    <div className="flex items-center gap-2.5" style={{ padding: '8px 14px' }}>
      ...
      <div className="flex-1 min-w-0">
        <div className="flex justify-between mb-1">
          <span className="text-[12.5px] font-extrabold" style={{ color: C.fg }}>{env.name}</span>
          <span
...
export function EnvelopesCard({ data }: { data: EnvelopesCardData }) {
  ...
  return (
    <div className="overflow-hidden">
      ...
      >
        <div>
          <p className="m-0 text-[10px] font-extrabold uppercase tracking-[.08em]" style={{ color: C.sub }}>
            {t('chat.envelopes.title')} · {monthLabel}
          </p>
          <p className="m-0 mt-0.5 text-[18px] font-black tabular-nums" style={{ color: C.fg, letterSpacing: -0.4 }}>
      ...
      {envelopes.length > 0 ? (
        ...
        <div className="py-4 text-center text-[13px] font-bold" style={{ color: C.sub }}>

 ...
 *   - Max 3 suggestions visible by default; "Ещё N" toggle for the rest
 *   - Max 1 split combo card (most recent)
 *   - Split CTA only shown when stage === 'split' OR amount >= SPLIT_AMOUNT_THRESHOLD
 *
...
export function ClarificationPanel({
  ...
  const visibleSuggestions = showAll ? suggestions : suggestions.slice(0, VISIBLE_DEFAULT);
  const hiddenCount = suggestions.length - VISIBLE_DEFAULT;

  const hasHabitSuggestion = suggestions.slice(0, VISIBLE_DEFAULT).some((s) =>
    s.reasons.some((r) => r.kind === 'habit'),
  );

  const showSplitCta = stage === 'split' || (amount != null && amount >= SPLIT_AMOUNT_THRESHOLD);
  const topCombo = splitCombos[0] ?? null;
  ...
  return (
    <div className="rounded-2xl bg-card border border-border overflow-hidden">
      ...
      <div className="px-3 py-2 space-y-1.5">
        ...
        {/* "Ещё N вариантов" toggle */}
        {!showAll && hiddenCount > 0 && (
          <button
            onClick={() => setShowAll(true)}
            className="w-full text-center text-[11px] font-bold text-muted-foreground hover:text-foreground py-1 transition-colors"
          >
            Ещё {hiddenCount} {hiddenCount === 1 ? 'вариант' : hiddenCount < 5 ? 'варианта' : 'вариантов'}
          </button>
        )}
      </div>

      {/* Single split combo card — most recent only */}
      {topCombo && onUseSplitCombo && (() => {
        const cats = topCombo.categoryIds
        ...
        return (
          <div className="px-3 pb-2">
            <p className="text-[10px] font-bold text-muted-foreground uppercase tracking-wider px-1 pb-1.5">
              Повторить сплит
            </p>
            <button
              onClick={() => onUseSplitCombo(topCombo)}
